...
export class CalendarComponent implements OnInit, AfterViewInit {
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode && this.selectedEvent) {
      ...
      this.http.put(`${this.apiUrl}/${this.selectedEvent.id}`, this.formData).subscribe({
        next: () => {
      ...
      this.http.post<Appointment>(this.apiUrl, this.formData).subscribe({
        next: () => {
          this.showModal = false;
          this.loadAppointments();
        },
  ...
  exportToExcel(): void {
    ...
    this.http.get<Appointment[]>(`${this.apiUrl}/range?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`).subscribe({
      next: (appointments) => {
